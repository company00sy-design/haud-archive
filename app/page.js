...
export default function HaudArchiveApp() {
  ...
  const deleteProject = async () => {
    if (!confirm("이 시공 기록을 삭제하시겠습니까? 복구할 수 없습니다.")) return;
    try {
      const { error } = await supabase.from('projects').delete().match({ id: selectedProject.id });
      if (error) throw error;
      alert('삭제되었습니다.');
      setIsDetailOpen(false);
      fetchProjects();
  ...
  return (
    <main className="max-w-6xl mx-auto p-4 md:p-10 bg-gray-50 min-h-screen pb-20 font-sans text-gray-900">
      ...
      <details className="bg-white p-6 rounded-[2.5rem] shadow-sm mb-12 border border-blue-50">
        ...
        <form onSubmit={handleSubmit} className="mt-8 space-y-6 pt-8 border-t border-gray-100 text-left px-2 text-sm">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="flex flex-col gap-1">
              ...
              <input type="text" placeholder="현장명을 입력하세요" value={formData.customer_name} className="p-4 rounded-2xl bg-gray-50 font-bold outline-none border-none" onChange={e => setFormData({...formData, customer_name: e.target.value})} />
            </div>
          </div>
          ...
          <textarea placeholder="기사 전달사항 / AS 메모" className="w-full p-5 rounded-[2rem] bg-gray-50 h-32 outline-none border-none shadow-inner font-medium" onChange={e => setFormData({...formData, as_note: e.target.value})} />
      ...
      <div className="mb-10 relative px-2">
        <input type="text" placeholder="현장명, 제품명, 담당자 이름 검색..." className="w-full p-6 pl-14 rounded-[2.5rem] border-none shadow-sm text-sm outline-none bg-white focus:ring-2 focus:ring-blue-100 transition-all" onChange={e => setSearchTerm(e.target.value)} />
          ...
          <div key={p.id} onClick={() => openDetail(p)} className="bg-white rounded-[3rem] overflow-hidden shadow-sm hover:shadow-xl transition-all cursor-pointer border border-white group">
            <div className="h-64 overflow-hidden relative bg-gray-100">
              {p.after_urls?.[0] ? (
                ...
                <div className="w-full h-full flex items-center justify-center text-gray-300 text-[10px] font-black uppercase italic">No Photo</div>
              )}
            ...
            <div className="p-8">
              ...
              <p className="text-xs font-bold text-blue-500 mb-4 uppercase">기사: {p.installer_name?.split('@')[0]}</p>
              <p className="text-xs font-bold text-gray-400 truncate mb-6 pb-4 border-b border-gray-50">{p.product_name || '상세 정보 입력 대기'}</p>
              <div className="flex justify-between items-center">
                <span className="text-[10px] font-black text-gray-300 tracking-widest uppercase">Detail Info</span>
        ...
        <div className="fixed inset-0 z-[150] bg-black/80 backdrop-blur-md flex items-center justify-center p-4" onClick={() => setIsDetailOpen(false)}>
          <div className="bg-white w-full max-w-5xl rounded-[3rem] shadow-2xl overflow-hidden relative animate-in zoom-in-95 duration-300 max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
            <div className="p-8 md:p-12 border-b flex justify-between items-start sticky top-0 bg-white z-10">
              <div>
                <span className="text-[10px] font-black text-blue-400 tracking-[0.2em] uppercase">Specification & Update</span>
            ...
            <div className="p-8 md:p-12 grid grid-cols-1 md:grid-cols-2 gap-12 text-sm">
              <div className="space-y-6">
                <p className="text-[11px] font-black text-gray-300 uppercase tracking-[0.3em]">Photo Gallery</p>
                ...
                <div className="p-6 bg-blue-50/50 rounded-[1.5rem] border-2 border-dashed border-blue-100 text-center">
                  <p className="text-[10px] font-black text-blue-800 mb-2 uppercase italic tracking-widest">➕ 사진 추가하기 (선택)</p>
                ...
                {selectedProject.as_note && (
                  <div className="p-6 bg-red-50 rounded-[2rem] border border-red-100">
                    <p className="text-[10px] font-black text-red-400 uppercase tracking-widest mb-2 italic underline underline-offset-4">Installer Memo</p>
                    <p className="text-[13px] text-red-900 font-semibold leading-relaxed">{selectedProject.as_note}</p>
                  </div>
                )}
              </div>

              <div className="flex flex-col h-full">
                <p className="text-[11px] font-black text-gray-300 uppercase tracking-[0.3em] mb-6">Case Specification</p>
                <div className="space-y-6 flex-1">
                  <div className="flex flex-col gap-1.5">
                    <label className="text-[11px] font-black text-blue-400 ml-1 uppercase italic">Product Name</label>
                    <input className="w-full p-5 bg-gray-50 rounded-2xl font-black border-none outline-none focus:ring-2 focus:ring-blue-100" value={editData.product_name || ''} placeholder="정보를 입력하세요" onChange={e => setEditData({...editData, product_name: e.target.value})} />
                  </div>
                  <div className="flex flex-col gap-1.5">
                    <label className="text-[11px] font-black text-blue-400 ml-1 uppercase italic">Sales Manager</label>
                    <input className="w-full p-5 bg-gray-50 rounded-2xl font-black border-none outline-none focus:ring-2 focus:ring-blue-100" value={editData.manager || ''} placeholder="담당자 이름" onChange={e => setEditData({...editData, manager: e.target.value})} />
                  </div>
                  <div className="flex flex-col gap-1.5">
                    <label className="text-[11px] font-black text-blue-400 ml-1 uppercase italic">Tags (쉼표 구분)</label>
                    <input className="w-full p-5 bg-gray-50 rounded-2xl font-black border-none outline-none focus:ring-2 focus:ring-blue-100" value={editData.tags || ''} placeholder="화이트, 무몰딩, 푸시" onChange={e => setEditData({...editData, tags: e.target.value})} />
                  </div>
                </div>

                <div className="flex gap-3 mt-12">
                  <button onClick={saveUpdate} disabled={loading} className="flex-[3] bg-blue-900 text-white p-6 rounded-[1.8rem] font-black text-base shadow-2xl shadow-blue-100 active:scale-95 transition-all">
                    {loading ? '업데이트 중...' : '정보 업데이트 저장'}
                  </button>
      ...
      {modalData.isOpen && (
        <div className="fixed inset-0 z-[200] bg-black/95 flex items-center justify-center animate-in fade-in duration-200" onClick={closePhotoModal}>
          <button className="absolute top-8 right-8 text-white text-5xl font-light" onClick={closePhotoModal}>&times;</button>
          <button className="absolute left-6 md:left-12 text-white/40 hover:text-white text-7xl p-2 transition-colors" onClick={prevImg}>&#8249;</button>
          <div className="max-w-[85%] max-h-[80%] flex flex-col items-center">
            ...
            <p className="text-white font-black text-xs mt-8 italic tracking-widest">{modalData.currentIndex + 1} / {modalData.images.length}</p>
          </div>
          <button className="absolute right-6 md:right-12 text-white/40 hover:text-white text-7xl p-2 transition-colors" onClick={nextImg}>&#8250;</button>
        </div>
      )}
    </main>
  )
