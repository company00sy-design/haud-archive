...
export default function HaudArchiveApp() {
  ...
  const saveUpdate = async () => {
    if (!selectedProject?.id) return alert("ID가 없어 업데이트가 불가합니다. DB 설정을 확인하세요.");
    setLoading(true);
    try {
  ...
  const deleteProject = async () => {
    if (!selectedProject?.id) return alert("ID가 없어 삭제가 불가합니다.");
    ...
    <main className="max-w-6xl mx-auto p-4 md:p-10 bg-gray-50 min-h-screen pb-20 font-sans text-gray-900">
      ...
      <details className="bg-white p-6 rounded-[2.5rem] shadow-sm mb-12 border border-blue-50">
        <summary className="font-bold text-blue-900 cursor-pointer list-none flex justify-between items-center py-2 focus:outline-none">
          <span className="text-lg font-black tracking-tight uppercase italic">➕ Register New Case</span>
        ...
        <form onSubmit={handleSubmit} className="mt-8 space-y-6 pt-8 border-t border-gray-100 text-left px-2">
          ...
          <textarea placeholder="특이사항 또는 AS 메모" className="w-full p-5 rounded-[2rem] bg-gray-50 h-32 outline-none border-none shadow-inner text-sm font-medium" onChange={e => setFormData({...formData, as_note: e.target.value})} />
      ...
      <div className="mb-10 relative px-2">
        <input type="text" placeholder="현장명, 제품명 검색..." className="w-full p-5 pl-14 rounded-[2.5rem] border-none shadow-sm text-sm outline-none bg-white focus:ring-2 focus:ring-blue-100 transition-all" onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredProjects.map((p) => (
          <div key={p.id} onClick={() => openDetail(p)} className="bg-white rounded-[3rem] overflow-hidden shadow-sm hover:shadow-xl transition-all cursor-pointer border border-white group flex flex-col">
            <div className="h-64 overflow-hidden relative bg-gray-100">
              {p.after_urls?.[0] ? <img src={p.after_urls[0]} className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-700" /> : <div className="w-full h-full flex items-center justify-center text-gray-300 text-[10px] font-bold uppercase italic tracking-widest">No Image</div>}
            ...
            <div className="p-8 flex-1 flex flex-col">
              <h3 className="text-2xl font-black text-gray-900 mb-1 tracking-tighter leading-tight uppercase group-hover:text-blue-900 transition-colors">{p.customer_name || '미등록 현장'}</h3>
              <p className="text-xs font-black text-blue-500 mb-4 uppercase italic">기사: {p.installer_name?.split('@')[0] || 'Unknown'}</p>
              <div className="mt-auto pt-5 border-t border-gray-50 flex justify-between items-center">
                <p className="text-xs font-bold text-gray-400 truncate flex-1 mr-4">{p.product_name || '상세 정보 대기'}</p>
                <span className="text-[11px] font-black text-blue-600 whitespace-nowrap uppercase">VIEW →</span>
              </div>
            </div>
          </div>
        ))}
        ...
        <div className="fixed inset-0 z-[150] bg-black/80 backdrop-blur-md flex items-center justify-center p-4" onClick={() => setIsDetailOpen(false)}>
          <div className="bg-white w-full max-w-5xl rounded-[3rem] shadow-2xl overflow-hidden relative animate-in zoom-in-95 duration-300 max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
            <div className="p-8 md:p-12 border-b flex justify-between items-start sticky top-0 bg-white z-10">
              <div>
                <span className="text-[10px] font-black text-blue-400 tracking-[0.2em] uppercase italic">{selectedProject.work_date} Case Study</span>
            ...
            <div className="p-8 md:p-12 grid grid-cols-1 md:grid-cols-2 gap-12 text-sm">
              ...
              <div className="flex flex-col h-full space-y-6">
                <div className="flex flex-col gap-1.5 font-bold">
                  ...
                  <input className="w-full p-5 bg-gray-50 rounded-2xl font-black border-none outline-none focus:ring-2 focus:ring-blue-100 text-base shadow-inner" value={editData.product_name || ''} placeholder="입력하세요" onChange={e => setEditData({...editData, product_name: e.target.value})} />
                </div>
                <div className="flex flex-col gap-1.5 font-bold">
                  <label className="text-[11px] font-black text-blue-400 ml-1 uppercase italic tracking-widest">Sales Manager</label>
                  <input className="w-full p-5 bg-gray-50 rounded-2xl font-black border-none outline-none focus:ring-2 focus:ring-blue-100 text-base shadow-inner" value={editData.manager || ''} placeholder="성함" onChange={e => setEditData({...editData, manager: e.target.value})} />
                </div>
                <div className="flex flex-col gap-1.5 font-bold">
                  <label className="text-[11px] font-black text-blue-400 ml-1 uppercase italic tracking-widest">Hash Tags</label>
                  <input className="w-full p-5 bg-gray-50 rounded-2xl font-black border-none outline-none focus:ring-2 focus:ring-blue-100 text-base shadow-inner italic" value={editData.tags || ''} placeholder="쉼표로 구분" onChange={e => setEditData({...editData, tags: e.target.value})} />
                </div>

                <div className="flex gap-3 mt-12">
                  <button onClick={saveUpdate} disabled={loading} className="flex-[3] bg-blue-900 text-white p-6 rounded-[1.8rem] font-black text-base shadow-2xl active:scale-95 transition-all">SAVE MODIFICATIONS</button>
      ...
      {modalData.isOpen && (
        <div className="fixed inset-0 z-[200] bg-black/95 flex items-center justify-center animate-in fade-in duration-200" onClick={closePhotoModal}>
          ...
          <button className="absolute left-6 text-white/40 hover:text-white text-7xl p-2 transition-all" onClick={prevImg}>&#8249;</button>
          ...
          <button className="absolute right-6 text-white/40 hover:text-white text-7xl p-2 transition-all" onClick={nextImg}>&#8250;</button>
        </div>
      )}
    </main>
  )
