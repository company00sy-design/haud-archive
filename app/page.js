...
export default function HaudArchiveApp() {
  ...
  // [개선] 통합 검색 로직: 현장명, 제품명, 태그, 시공기사를 모두 검색
  const filteredProjects = projects.filter(p => {
    const searchLow = searchTerm.toLowerCase()
    return (
      (p.customer_name || "").toLowerCase().includes(searchLow) ||
      (p.product_name || "").toLowerCase().includes(searchLow) ||
      (p.tags || "").toLowerCase().includes(searchLow) ||
      (p.installer_name || "").toLowerCase().includes(searchLow) ||
      (p.manager || "").toLowerCase().includes(searchLow)
    )
  })
  ...
  if (!user) return (
    <main className="max-w-md mx-auto p-10 flex flex-col justify-center min-h-screen font-sans selection:bg-blue-100">
      ...
      <form onSubmit={handleLogin} className="space-y-4">
        ...
        <button className="w-full bg-blue-900 text-white p-5 rounded-2xl font-bold text-lg shadow-xl hover:bg-blue-800 transition-all">로그인</button>
      </form>
    </main>
  )

  return (
    <main className="max-w-6xl mx-auto p-4 md:p-10 bg-gray-50 min-h-screen pb-20 font-sans selection:bg-blue-100">
      <style jsx global>{` @import url('https://cdn.jsdelivr.net/gh/orioncactus/pretendard/dist/web/static/pretendard.css'); body { font-family: 'Pretendard', sans-serif; } `}</style>
      ...
      <details className="bg-white p-6 rounded-[2.5rem] shadow-sm mb-12 border border-blue-50 overflow-hidden">
        <summary className="font-bold text-blue-900 cursor-pointer list-none flex justify-between items-center py-2 px-2 focus:outline-none">
          ...
          <span className="text-xs text-blue-500 font-bold">열기</span>
        </summary>
      ...
      <div className="mb-10 relative px-2">
        <input type="text" placeholder="현장명, 제품명, 태그(#), 담당자로 검색..." className="w-full p-6 pl-14 rounded-[2.5rem] border-none shadow-sm text-sm outline-none bg-white focus:ring-2 focus:ring-blue-100 transition-all font-bold" onChange={e => setSearchTerm(e.target.value)} />
        ...
        {filteredProjects.map((p) => (
          <div key={p.id} onClick={() => openDetail(p)} className="bg-white rounded-[3rem] overflow-hidden shadow-sm hover:shadow-xl transition-all cursor-pointer border border-white group flex flex-col">
            ...
            <div className="p-8 flex-1 flex flex-col">
              ...
              <p className="text-xs font-bold text-blue-500 mb-4">시공: {p.installer_name?.split('@')[0]}</p>
              <div className="mt-auto pt-5 border-t border-gray-50 flex justify-between items-center font-bold">
                <p className="text-gray-400 text-xs truncate flex-1 mr-4">{p.product_name || '상세 정보 입력 전'}</p>
                <span className="text-blue-600 text-xs">상세보기 →</span>
              </div>
            </div>
          </div>
        ))}
        ...
        <div className="fixed inset-0 z-[150] bg-black/80 backdrop-blur-md flex items-center justify-center p-4" onClick={() => setIsDetailOpen(false)}>
          <div className="bg-white w-full max-w-5xl rounded-[3rem] shadow-2xl overflow-hidden relative max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
            <div className="p-8 md:p-12 border-b flex justify-between items-start sticky top-0 bg-white z-10 font-bold">
              <div>
                <span className="text-xs text-blue-400 font-black">{selectedProject.work_date} 시공 상세</span>
            ...
            <div className="p-8 md:p-12 grid grid-cols-1 md:grid-cols-2 gap-12 text-sm font-bold">
              <div className="space-y-6">
                ...
                <div className="p-6 bg-blue-50/50 rounded-[1.5rem] border-2 border-dashed border-blue-100 text-center text-xs font-black text-blue-800">
                  <p className="mb-2">➕ 사진 추가 업로드</p>
                  <input type="file" id="extra_imgs" multiple accept="image/*" className="w-full" />
                </div>
              </div>
              <div className="flex flex-col space-y-6">
                <div className="flex flex-col gap-1.5"><label className="text-xs font-black text-blue-400">시공 제품명</label><input className="w-full p-5 bg-gray-50 rounded-2xl font-bold outline-none border-none shadow-inner" value={editData.product_name || ''} placeholder="예: 무몰딩 붙박이장" onChange={e => setEditData({...editData, product_name: e.target.value})} /></div>
                <div className="flex flex-col gap-1.5"><label className="text-xs font-black text-blue-400">영업 담당자</label><input className="w-full p-5 bg-gray-50 rounded-2xl font-bold outline-none border-none shadow-inner" value={editData.manager || ''} placeholder="담당자 성함" onChange={e => setEditData({...editData, manager: e.target.value})} /></div>
                <div className="flex flex-col gap-1.5"><label className="text-xs font-black text-blue-400">검색 태그</label><input className="w-full p-5 bg-gray-50 rounded-2xl font-bold outline-none border-none shadow-inner" value={editData.tags || ''} placeholder="예: 무몰딩, 화이트, 푸시 (쉼표 구분)" onChange={e => setEditData({...editData, tags: e.target.value})} /></div>
                <div className="flex gap-3 mt-auto pt-10 font-black">
                  <button onClick={saveUpdate} disabled={loading} className="flex-[3] bg-blue-900 text-white p-6 rounded-[1.8rem] shadow-xl active:scale-95 transition-all">수정 내용 저장</button>
                  <button onClick={deleteProject} className="flex-1 bg-red-50 text-red-600 p-6 rounded-[1.8rem] hover:bg-red-600 hover:text-white transition-all shadow-sm text-xs font-black">삭제</button>
                </div>
              </div>
            </div>
          </div>
