...
export default function HaudArchiveApp() {
  ...
  // [핵심 해결 로직] Supabase jsonb 규격에 완벽히 맞춘 데이터 전송
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const { error } = await supabase.from('projects').insert([{
        ...
        // jsonb 타입 그릇에 맞게 배열 그대로 전송 (null 방지)
  ...
  return (
    <main className="max-w-6xl mx-auto p-4 md:p-10 bg-gray-50 min-h-screen pb-20 font-sans text-gray-900 transition-all">
      <header className="flex justify-between items-end mb-10 py-4 border-b-2 border-gray-100 px-2 font-black italic">
        <div>
          <h1 className="text-3xl text-blue-900 uppercase italic tracking-tighter">hAUD ARCHIVE</h1>
        ...
        {filteredProjects.map((p) => (
          <div key={p.id} onClick={() => openDetail(p)} className="bg-white rounded-[3rem] overflow-hidden shadow-sm hover:shadow-xl transition-all cursor-pointer border border-white group flex flex-col">
            ...
            <div className="p-8 flex-1 flex flex-col">
              <h3 className="text-2xl font-black text-gray-800 mb-1 leading-tight uppercase group-hover:text-blue-900">{p.customer_name || '미등록'}</h3>
              <p className="text-xs font-black text-blue-500 mb-4 uppercase italic tracking-tighter">기사: {p.installer_name?.split('@')[0]}</p>
        ...
        <div className="fixed inset-0 z-[150] bg-black/80 backdrop-blur-md flex items-center justify-center p-4" onClick={() => setIsDetailOpen(false)}>
          <div className="bg-white w-full max-w-5xl rounded-[3rem] shadow-2xl overflow-hidden relative max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
            <div className="p-8 md:p-12 border-b flex justify-between items-start sticky top-0 bg-white z-10">
              <div>
                <span className="text-[10px] font-black text-blue-400 uppercase italic">{selectedProject.work_date} Case Detail</span>
            ...
            <div className="p-8 md:p-12 grid grid-cols-1 md:grid-cols-2 gap-12 text-sm">
              <div className="space-y-6">
                <p className="text-[11px] font-black text-gray-300 uppercase italic">Gallery</p>
              ...
              <div className="flex flex-col space-y-6">
                ...
                <div className="flex gap-3 mt-auto pt-10 font-black italic">
                  ...
                  <button onClick={deleteProject} className="flex-1 bg-red-50 text-red-600 p-6 rounded-[1.8rem] hover:bg-red-600 hover:text-white transition-all shadow-sm text-[10px] uppercase">Delete</button>
                </div>
              </div>
            </div>
          </div>
